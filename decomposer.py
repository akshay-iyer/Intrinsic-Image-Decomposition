...
for epoch in range(args.num_epochs):
    ...
    torch.save( state, open(os.path.join(args.save_path, 'state_dropout_new.t7'), 'wb') )
    ...
    # refl_es.check_early_stopping(train_losses[0] ,val_losses[0],model,trainer.optimizer, epoch, args.save_path)
    # shape_es.check_early_stopping(train_losses[0],val_losses[1],model,trainer.optimizer, epoch, args.save_path)
    # light_es.check_early_stopping(train_losses[0],val_losses[2],model,trainer.optimizer, epoch, args.save_path)

    # if refl_es.early_stop or shape_es.early_stop or light_es.early_stop:
